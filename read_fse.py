...
def logs_init(_filename): 
    ...
    try:
        with open('logs/' + _filename,"w") as file:
...
def indicate_status(forces):
    ...
    GPIO.output(_positive_x_pin, (forces[1] > force_threshold))
    GPIO.output(_negative_x_pin, (forces[1] < (force_threshold * -1)))
    GPIO.output(_positive_y_pin, (forces[2] > force_threshold))
    GPIO.output(_negative_y_pin, (forces[2] < (force_threshold * -1)))
    GPIO.output(_positive_z_pin, (forces[3] > force_threshold))
    GPIO.output(_negative_z_pin, (forces[3] < (force_threshold * -1)))
...
def run_script(program_timeout):
    ...
    while (current_time - start_time).total_seconds() < (program_timeout * 60):
...
if __name__ == "__main__":
    ...
    try:
        ...
        s = Serial(port=port, baudrate=115200, timeout=0.1, bytesize=EIGHTBITS,
        ...
        print("Error Initializing CLI")
        


    ####################################################
    #                       TODOs                      #
    # - change all vars from camelCase to under_scores #
    # - clean up pseudocode
    # - 
